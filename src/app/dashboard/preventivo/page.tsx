...
export default function PreventivoPage() {
  ...
  const handleSubmit = async (data: Record<string, string>) => {
    const response = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}/api/genera`, {
      ...
      body: JSON.stringify({ tipo: "preventivo", campi: data, titolo: `Preventivo - ${data.cliente || new Date().toLocaleDateString("it-IT")}` }),
    });
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-8">
        ...
        <p className="text-sm text-[#6B7280] mt-1">Crea preventivi professionali per i tuoi clienti</p>
      ...
      <TemplateForm fields={[
        ...
        { name: "professionista", label: "Tua professione / Studio", type: "text", placeholder: "Es: Dott. Marco Rossi", required: true },
        { name: "servizio", label: "Servizio da preventivare", type: "textarea", placeholder: "Descrivi il servizio", required: true, minHeight: 100 },
        { name: "importo", label: "Importo totale (€)", type: "number", placeholder: "Es: 1500", required: true },
        { name: "note", label: "Note aggiuntive", type: "textarea", placeholder: "Modalità di pagamento, condizioni particolari" },
      ]} onSubmit={handleSubmit} buttonText="Genera Preventivo" />
    </div>
  );
}